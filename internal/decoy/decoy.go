...
func (d *decoy) sendLoopPacket(doc *pki.Document, recipient []byte, src, dst *pki.MixDescriptor) {
	...
	for attempts := 0; attempts < maxAttempts; attempts++ {
		...
		fwdPath, then, err := d.makePath(doc, recipient, src, dst, &surbID, time.Now(), true)
		if err != nil {
			d.log.Debugf("Failed to select forward path: %v", err)
			return
		}

		revPath, then, err := d.makePath(doc, d.recipient, dst, src, &surbID, then, false)
		if err != nil {
...
func (d *decoy) sendDiscardPacket(doc *pki.Document, recipient []byte, src, dst *pki.MixDescriptor) {
	...
	for attempts := 0; attempts < maxAttempts; attempts++ {
		...
		fwdPath, then, err := d.makePath(doc, recipient, src, dst, nil, time.Now(), true)
		if err != nil {
			d.log.Debugf("Failed to select forward path: %v", err)
			return
		}
...
func (d *decoy) makePath(doc *pki.Document, recipient []byte, src, dst *pki.MixDescriptor, surbID *[sConstants.SURBIDLength]byte, baseTime time.Time, isForward bool) ([]*sphinx.PathHop, time.Time, error) {

	// This is essentially identical to how minclient handles path creation
	// with the following differences:
	//
	//  * The source/destination are specified as descriptors.
	//  * The recipient is specified as a byte slice.
	//  * It allows paths that do not span the entire network.
	//  * It gives up after a few attempts.
	//
	// Note: Forward paths MUST terminate at a Provider, reverse paths MAY
	// terminate at a mix.
...
selectLoop:
	for attempts := 0; attempts < maxAttempts; attempts++ {
		descs, err := selectHops(d.rng, doc, src, dst, isForward)
		if err != nil {
		...
		for idx, desc := range descs {
			...
			if k, ok := desc.MixKeys[epoch]; !ok {
				d.log.Debugf("Hop[%v]: Node %v missing mixkey for epoch %v", idx, desc.IdentityKey, epoch)
				continue selectLoop
			} else {
				h.PublicKey = k
			}
			...
			if idx != len(descs)-1 || (surbID != nil && isForward) {
				delay = uint64(rand.Exp(d.rng, doc.MixLambda)) + 1
			...
			d.log.Debugf("Hop[%v]: '%v' - %d ms", idx, desc.Name, delay)
			path = append(path, h)
		}
...
func selectHops(rng *mRand.Rand, doc *pki.Document, src, dst *pki.MixDescriptor, isForward bool) ([]*pki.MixDescriptor, error) {
	var hops []*pki.MixDescriptor

	var startLayer, nHops int
	if isForward {
		if dst.Layer != pki.LayerProvider {
			return nil, fmt.Errorf("invalid destination layer: %v", dst.Layer)
		}
		...
		if src.Layer != pki.LayerProvider {
			return nil, fmt.Errorf("invalid source layer: %v", src.Layer)
		}
	...
	for i, nodes := range doc.Topology[startLayer:] {
		...
		if len(nodes) == 0 {
			return nil, fmt.Errorf("layer %v has no nodes", i)
		}
